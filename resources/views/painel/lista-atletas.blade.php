...
@section('main')
  ...
  <div class="d-flex justify-content-between mb-4">
    <h1 class="h3">Atletas cadastrados</h1>
  </div>
  ...
  <div class="d-flex justify-content-between align-items-end mb-3">
    <form action="/painel/atletas" method="GET" class="bg-custom rounded col-12 py-3 px-4">
        
      <div class="row align-items-end row-gap-4">
        <div class="col-5 d-flex flex-wrap">
          <label for="nome" class="col-form-label">Nome:</label>
          <div class="col-12">
            <input type="text" class="form-control bg-dark text-light border-dark" id="nome" name="nome" placeholder="Busca por nome">
          </div>
        </div>
        ...
        <div class="col-5 d-flex flex-wrap">
          ...
          <div class="col-12">
            <input type="text" class="form-control bg-dark text-light border-dark" id="email" name="email" placeholder="Busca por email">
          </div>
        </div>
    ...
    <table class="table mb-0 table-custom table-dark align-middle">
      <thead>
        <tr>
          ...
          <th scope="col" class="text-uppercase">Nascimento</th>
          <th scope="col" class="text-uppercase">Equipe</th>
      ...
      <tbody>
        @foreach ($atletas as $atleta)
          <tr>
            ...
            <td>{{ $atleta->nome }}</td>
            <td>{{ $atleta->email }}</td>
            <td>{{ date('d/m/Y', strtotime($atleta->nascimento)) }}</td>
            <td>{{ $atleta->equipe }}</td>
            <td>
              <div class="d-flex justify-content-center">
                <button type="button" class="btn btn-light d-flex justify-content-center align-items-center rounded-circle p-2 mx-2" data-bs-toggle="modal" data-bs-target="#modal{{ $atleta->id }}">
                ...
                @can('admin')
                  <a href="/painel/edicao_atleta/{{$atleta->id}}" class="btn btn-light d-flex justify-content-center align-items-center rounded-circle p-2 mx-2" title="Editar">
                  ...
                  <form action="/painel/atletas/{{ $atleta->id }}" method="POST">
                    @csrf
                    @method('DELETE')
...
@section('modals')
  @foreach ($atletas as $atleta)
    <div class="modal fade" id="modal{{ $atleta->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered text-light">
        <div class="modal-content bg-custom">
          ...
          <div class="modal-body d-flex flex-wrap row-gap-4">
            ...
            <div class="col-6">
              <div><small style="text-decoration: underline">Nome:</small></div>
              <div style="color: orange">{{ $atleta->nome }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Nascimento:</small></div>
              <div style="color: orange">{{ date('d/m/Y', strtotime($atleta->nascimento)) }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">E-mail:</small></div>
              <div style="color: orange">{{ $atleta->email }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Equipe:</small></div>
              <div style="color: orange">{{ $atleta->equipe }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Sexo:</small></div>
            ...
            <div class="col-6">
              ...
              <div style="color: orange">{{ date('d/m/Y, H:i:s', strtotime($atleta->created_at)) }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Última Alteração:</small></div>
              <div style="color: orange">{{ date('d/m/Y, H:i:s', strtotime($atleta->updated_at)) }}</div>
            </div>
            @endcan
