...
@section('op') {{-- OPÇÕES DO MENU LATERAL (INCÍCIO) --}}
  ...
  <div class="item">
    ...
    <div class="collapse show" id="menu-usuario">
      <div class="bg-dark d-flex flex-column rounded mx-4 p-2 row-gap-1">
        <a href="/painel/users" class="submenu-link link-light text-decoration-none rounded p-2 active">
          <small class="d-flex justify-content-between align-items-center">
            Listagem
...
@section('main')
  ...
  <div class="d-flex justify-content-between mb-4">
    ...
    <div class="d-flex gap-2">
      <a href="/painel/registro_usuario" class="btn btn-light">+ Novo usuário</a>
    </div>
    @endcan
  </div>
  ...
  <div class="d-flex justify-content-between align-items-end mb-3">
    <form action="/painel/users" method="GET" class="bg-custom rounded col-12 py-3 px-4">
        
      <div class="row align-items-end row-gap-4">
        <div class="col-5 d-flex flex-wrap">
          <label for="name" class="col-form-label">Nome:</label>
          <div class="col-12">
            <input type="text" class="form-control bg-dark text-light border-dark" id="name" name="name" placeholder="Busca por nome">
          </div>
        </div>
    ...
    <table class="table mb-0 table-custom table-dark align-middle">
      <thead>
        <tr>
          ...
          <th scope="col" class="text-uppercase">Data de registro</th>
          <th scope="col" class="text-uppercase">Privilégio</th>
          ...
          <tr>
            ...
            <td>
              <div class="d-flex justify-content-center">
                <button type="button" class="btn btn-light d-flex justify-content-center align-items-center rounded-circle p-2 mx-2" data-bs-toggle="modal" data-bs-target="#modal{{ $user->id }}">
                ...
                @can('admin')
                  <a href="/painel/edicao_user/{{$user->id}}" class="btn btn-light d-flex justify-content-center align-items-center rounded-circle p-2 mx-2" title="Editar">
                  ...
                  <form action="/painel/users/{{ $user->id }}" method="POST">
                    @csrf
                    @method('DELETE')
        ...
        <div class="modal-content bg-custom">
          ...
          <div class="modal-body d-flex flex-wrap row-gap-4">
            ...
            <div class="col-6">
              ...
              <div style="color: orange">{{ $user->email }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Nível de acesso:</small></div>
              <div style="color: orange">{{ $user->is_admin == 1 ? 'Administrador' : 'Usuário' }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">ID:</small></div>
            ...
            <div class="col-6">
              ...
              <div style="color: orange">{{ date('d/m/Y, H:i:s', strtotime($user->created_at)) }}</div>
            </div>

            <div class="col-6">
              <div><small style="text-decoration: underline">Última Alteração:</small></div>
              <div style="color: orange">{{ date('d/m/Y, H:i:s', strtotime($user->updated_at)) }}</div>
            </div>
            @endcan
